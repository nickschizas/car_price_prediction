...
# Shape of the data
shape = data.shape
st.sidebar.markdown(f'*{data.shape[0]:,.0f} rows of data*')
...
# Klm - Price - Gas Type
with main_col[1]:
    variable = st.selectbox('Scatterplot variable', options=['Klm', 'CubicCentimetres', 'Horsepower', 'Age'], key='scatter1')    
    filter_col = st.columns(3)
    with filter_col[0]:
        name_options = ['All'] + list(data['Name'].unique())
        name = st.selectbox('Brand', options=name_options, key='scatter2')
    with filter_col[1]:
        gas_type_options = ['All'] + list(data['GasType'].unique())
        gas_type = st.selectbox('Gas Type', options=gas_type_options, key='scatter3')
    with filter_col[2]:
        percentile = st.number_input('Percentile (0-100)', min_value=0, max_value=100, value=100, key='scatter4')
